    ...
    public async makePayment(user: User, planId: string) {
        ...
        if(user.id === plan.channel.id) {
            throw new ConflictException(
                'You cannot subscribe to your own sponsorship'
            )
        }
        ...
        const session = await this.stripeService.checkout.sessions.create({
            ...
            mode: 'subscription',
            success_url: `${this.configService.getOrThrow<string>('ALLOWED_ORIGIN')}/success?price=${plan.
                title}&username=${plan.channel.username}`,
